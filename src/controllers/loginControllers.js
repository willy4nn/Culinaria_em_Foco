    ...
    const user = await jwt.verify(sessionToken, config.SECRET_KEY, (err, decoded) => {
        if (err) {
            ...
            if (!decoded) {
                return res
                    .status(403)
                    .json({ error: "Token JWT inválido, faça login novamente" })
...
const addUser = async (req, res) => {
    ...
    try {
        ...
        await client.query(
            "INSERT INTO users (name, username, email, password, user_type, premium_active, premium_date, profile_photo, created_at) VALUES ($1, $2, $3, $4, $5, $6, $7, $8, $9)",
            [
                newUser.name,
                newUser.username,
                newUser.email,
                hashedPassword,
                newUser.user_type,
                newUser.premium_active,
                newUser.premium_date,
...
const updateUser = async (req, res) => {
    ...
    if (sessionToken && userType.includes("admin")) {
        ...
        try {
            ...
            await client.query(
                "UPDATE users SET name = $1, username = $2, email = $3, password = $4, user_type = $5, premium_active = $6, premium_date = $7, profile_photo = $8, created_at = $9 WHERE username = $10",
                [
                    newUser.name,
                    newUser.username,
                    newUser.email,
